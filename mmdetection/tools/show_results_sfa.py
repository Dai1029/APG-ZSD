...
config_file = '/media/dxm/D/DXM/Model/ZSD-COCO/mmdetection/configs/faster_rcnn_r101_fpn_1x.py'
checkpoint_file = '/media/dxm/D/DXM/Model/ZSD-COCO/faster-pth/MSCOCO2014_epoch_12.pth'
# checkpoint_file = 'cp_48_17/epoch_12.pth'
syn_weights = '/media/dxm/D/DXM/Model/ZSD-COCO/Temp/66.pth'

score_thr = 0.45
try:
    os.makedirs('/media/dxm/D/DXM/SFA-GZSD-45/')
except OSError:
    pass
...
for idx, info in enumerate(img_infos):
    img = f"{root}/{info['filename']}"
    result = inference_detector(model, img)
    out_file = f"/media/dxm/D/DXM/SFA-GZSD-45/{img.split('/')[-1]}"
    show_result(f"{img}", result, model.CLASSES, out_file=out_file, show=False, score_thr=score_thr)
    print(f"[{idx:03}/{len(img_infos)}]")

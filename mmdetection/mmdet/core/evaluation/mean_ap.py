        ...
        for cls_result in eval_results:
            if cls_result['num_gts'] > 0:
                ...
                if cls_result['recall'].size > 0:
                    ...
                    recalls.append(cls_result['recall'][-1])

        mean_ap = np.array(aps).mean().item() if aps else 0.0
        # mean_recall = np.array(recalls).mean().item() if recalls else 0.0
    if print_summary:
        print_map_summary(aps, eval_results, dataset, dataset_name, area_ranges, recalls=recalls, split=split)

    return mean_ap, eval_results


def print_map_summary(aps, results, dataset=None, dataset_name=None, ranges=None, recalls=None, split='65_15'):
    ...
    if dataset_name is not None:   
        # print(dataset_name) 
        unseen_class_inds = get_unseen_class_ids(dataset=dataset_name, split=split)-1
        seen_class_inds = get_seen_class_ids(dataset=dataset_name, split=split)-1
    ...
    for i in range(num_scales):
        ...
        for ind in range(len(indexes)):
            ...
            mean_ap = np.array(curr_aps).mean().item() if len(curr_aps) > 0 else 0.0
            mean_recall = np.array(curr_recalls).mean().item() if len(curr_recalls) > 0 else 0.0
            table_data.append(['mean', '', '', '{:.3f}'.format(mean_recall), '', '{:.3f}'.format(mean_ap)])
            means_table.append(['mean', '', '', '{:.3f}'.format(mean_recall), '', '{:.3f}'.format(mean_ap)])

...
class TrainCls():
    ...
    def initDataSet(self, features, labels):
        self.dataset = FeaturesCls(self.opt, features=features, labels=labels, split='train',
                                   classes_to_train=self.classes_to_train)
        self.test_dataset = FeaturesCls(self.opt, split='test', classes_to_train=self.classes_to_train)
        self.dataloader = DataLoader(dataset=self.dataset, batch_size=self.opt.batch_size, num_workers=4, shuffle=True,
                                     pin_memory=True)
        self.test_dataloader = DataLoader(dataset=self.test_dataset, batch_size=self.opt.batch_size * 50, num_workers=4,
    ...
    def __call__(self, features=None, labels=None, gan_epoch=0):
        ...
        if self.dataset is None:
            ...
            self.valacc, self.all_acc, _ = val(self.test_dataloader, self.classifier, self.criterion, self.opt, 0,
                                               verbose="Test")
    ...
    def margin_loss(self, preds, in_label):
        ...
        for i in class_uni:
            ...
            pwd_loss += F.relu(0.5 - max_loss + min_loss)
    ...
    def trainEpochs(self):
        for epoch in range(self.opt.nepoch_cls):
            ...
            for ite, (in_feat, in_label) in enumerate(self.dataloader):
                ...
                if epoch+1 % 10 == 0:
                    if ite + 1 % 100 == 0 or ite + 1 == len(self.dataloader):
                        # print(f'Cls Train Epoch [{epoch + 1:02}/{self.opt.nepoch_cls}]
                        # Iter [{ite + 1 :04}/{len(self.dataloader)}] {(ite + 1) * 100 / len(self.dataloader):0.2f}%
                        # Loss: {loss_epoch / ite :0.4f} Loss-cls: {loss_cls :0.4f} Loss-m: {loss_m :0.4f}')
                        print(f'Cls Train Epoch [{epoch + 1:02}/{self.opt.nepoch_cls}] '
                              f'Iter [{ite + 1 :04}/{len(self.dataloader)}] {(ite + 1) * 100 / len(self.dataloader):0.2f}% '
                              f'Loss: {loss_epoch / ite :0.4f} Loss-cls: {loss :0.4f}')
            ...
            if self.best_acc <= self.valacc:
                ...
                self.best_epoch_cls = epoch+1

            if self.best_acc_now_epoch <= self.valacc:
                self.best_acc_now_epoch = self.valacc
                self.best_cls_epoch = epoch + 1
                print(f"saved [{self.gan_epoch + 1}] epoch the best test accuracy in [{epoch + 1}]: {self.best_acc_now_epoch:0.4f}")
                torch.save({'state_dict': self.classifier.state_dict(), 'epoch': epoch},
                           f"{self.opt.outname}/{self.gan_epoch + 1}.pth")

            # todo  find the best train epoch
        #     if epoch + 1 > 5:
        #         train_acc, _, _ = compute_per_class_acc(np.concatenate(gt_all), np.concatenate(preds_all), self.opt, verbose='Train')
        #         if self.best_train_acc <= train_acc:
        #             self.best_train_acc = train_acc
        #             self.best_train_epoch = epoch + 1
        #             print(f"saved best train model in [{self.best_train_epoch}] : {self.best_train_acc:0.4f}")
        #             torch.save({'state_dict': self.classifier.state_dict(), 'epoch': epoch},
        #                        f"{self.opt.outname}/train_best/{self.gan_epoch + 1}.pth")
        #
        # print(f"the model best train accuracy in [{self.best_train_epoch:02}] epoch: {self.best_train_acc:0.4f}")
        print(f"the model best test accuracy in [{self.best_cls_epoch:02}] epoch: {self.best_acc_now_epoch:0.4f}")
        print(f"[{self.best_epoch:03}] best model in [{self.best_epoch_cls:03}] accuracy: {self.best_acc}")
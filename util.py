...
def val(dataloader, classifier, criterion, opt, epoch, verbose="Val"):
    ...
    for ite, (in_feat, in_label) in enumerate(dataloader):
        ...
        if ite % 100 == 99:
            ...
            print(f'{verbose} Epoch [{epoch + 1:02}/{opt.nepoch_cls}] Iter [{ite + 1:03}/{len(dataloader)}]'
                  f' {(ite + 1) * 100 / len(dataloader):.2f}% Loss: {loss_epoch / ite:.4f}')
            # todo dxm
...
def compute_per_class_acc(test_label, predicted_label, opt, verbose="Val"):
    ...
    for index, label in enumerate(class_labels):
        ...
        acc_per_class[index] = 100 * torch.sum(test_label[idx] == predicted_label[idx]).float() / torch.sum(idx).float()

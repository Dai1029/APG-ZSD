...
class TrainGAN():
    ...
    def __call__(self, epoch, features, labels, type=0):
        """
        ...
        if type == 0:
            ...
            self.diff_feature = np.load("Diffusion_Feature/all_unseen_1024.npy")
            self.trainEpoch_with_Diff(self.opt.unseen_num, self.opt.cfxs)
        elif type == 2:
            self.diff_feature = np.load("Diffusion_Feature/voc_1024.npy")
            self.trainEpoch_with_Diff(self.opt.unseen_num, self.opt.cfxs)
    ...
    def generate_syn_feature(self, labels, attribute, num=100, fcl=1.0, no_grad=True):
        """
        generates features
        inputs:
    ...
    def generate_syn_feature_without(self, labels, attribute, num=100, fcl=1.0, no_grad=True):
        """
        generates features
        inputs:
            labels: features labels to generate nx1 n is number of objects
        ...
        unseen_feature = np.load("Stable_Clip_rn50/stable_diff_rn50.npy")
        if self.opt.cuda:
            syn_att = syn_att.cuda()
            syn_noise = syn_noise.cuda()
        if no_grad is True:
            with torch.no_grad():
                for i in range(nclass):
                    ...
                    syn_label.narrow(0, i * num, num).fill_(label)
                    if i == 11 or i == 0:
                        output = torch.from_numpy(unseen_feature[300*(i-5):300*(i-4)])
                        syn_feature.narrow(0, i * num, num).copy_(output)
                    else:
                        iclass_att = attribute[i]
                        syn_att.copy_(iclass_att.repeat(num, 1))
                        syn_noise.normal_(0, fcl)
    ...
    def getBatchUnseen(self, num=5):

        fg_inds = np.random.permutation(300)
        inds = np.random.permutation(fg_inds)[:int(num)]

        # coco
        label = [5, 7, 13, 16, 22, 29, 30, 32, 43, 49, 53, 62, 65, 71, 79]
        syn_feature = torch.FloatTensor(15 * num, 1024)
        syn_label = torch.LongTensor(15 * num)

        for i in range(15):
            syn_label.narrow(0, i * num, num).fill_(label[i])
            tmp = self.diff_feature[i*300:(i+1)*300][inds]
            output = torch.from_numpy(tmp)
            syn_feature.narrow(0, i * num, num).copy_(output)

        # voc
        # label = [17, 18, 19, 20]
        # syn_feature = torch.FloatTensor(4 * num, 1024)
        # syn_label = torch.LongTensor(4 * num)
        #
        # for i in range(4):
        #     syn_label.narrow(0, i * num, num).fill_(label[i])
        #     tmp = self.diff_feature[i*300:(i+1)*300][inds]
        ...
        syn_att = torch.from_numpy(self.attributes[syn_label])
        return syn_feature, syn_label, syn_att

    def calc_gradient_penalty(self, real_data, fake_data, input_att):
        len_batch = real_data.shape[0]

        alpha = torch.rand(len_batch, 1)
    ...
    def get_z_random(self, type=0):
        ...
        if type == 0:
            ...
            z = torch.cuda.FloatTensor(type, self.opt.nz)
            z.normal_(0, 1)
        return z
    ...
    def trainEpoch(self):
        for i in range(0, self.ntrain, self.opt.batch_size):
            ...
            if self.epoch >= 5:
                ...
                errG = G_cost + c_errG + loss_lz
            errG.backward()
            self.optimizerG.step()
            if self.epoch % 20 == 0:
                if i % (self.opt.batch_size * 100) == 0:
                    print(f"{self.gen_type} [{self.epoch + 1:02}/{self.opt.nepoch:02}] [{i:05}/{int(self.ntrain)}] "
                          f"Loss:{errG.item():06.4f} D-loss:{D_cost.data.item():06.4f} "
                          f"G-loss:{G_cost.data.item():06.4f} "
                          f"W-dist:{Wasserstein_D.data.item():06.4f} "
                          f"seen-loss:{c_errG.data.item():06.4f} unseen-loss:{unseenc_errG.data.item():06.4f} "
                          f"loss-div:{loss_lz.item():06.4f}")
    ...
    def trainEpoch_with_Diff(self, unseen_num=24, cfxs=1.4):
        for i in range(0, self.ntrain, self.opt.batch_size):
            # import pdb; pdb.set_trace()
            input_res, input_label, input_att = self.sample()  # take batch_size seen feature label att
            diff_res, diff_label, diff_att = self.getBatchUnseen(unseen_num)
            ...
            diff_res, diff_label, diff_att = diff_res.type(torch.FloatTensor).cuda(), diff_label.type(
                torch.LongTensor).cuda(), diff_att.type(torch.FloatTensor).cuda()
            ...
            for iter_d in range(self.opt.critic_iter):
                ...
                # todo dxm
                # train with realG
                diff_resv = Variable(diff_res)
                diff_attv = Variable(diff_att)

                criticD_diff_r = self.netD(diff_resv, diff_attv)
                criticD_diff_r = criticD_diff_r.mean()

                # train with fakeG
                noise = self.get_z_random(15*unseen_num)
                noisev = Variable(noise)
                fake = self.netG(noisev, diff_attv)
                criticD_diff_f = self.netD(fake.detach(), diff_attv)
                criticD_diff_f = criticD_diff_f.mean()

                # gradient penalty
                gradient_penalty_dif = self.calc_gradient_penalty(diff_res, fake.data, diff_att)
                fake_D = criticD_diff_r - criticD_diff_f

                # todo dxm    unseen_5 惩罚系数
                D_cost_unseen = cfxs * (criticD_diff_f - criticD_diff_r + gradient_penalty_dif)
                D_cost = criticD_fake - criticD_real + gradient_penalty + D_cost_unseen
            ...
            if self.epoch % 20 == 0:
                if i % (self.opt.batch_size * 100) == 0:
                    print(f"{self.gen_type} [{self.epoch + 1:02}/{self.opt.nepoch:02}] [{i:05}/{int(self.ntrain)}] "
                          f"Loss:{errG.item():06.4f} D-loss:{D_cost.data.item():06.4f} G-loss:{G_cost.data.item():06.4f} "
                          f"W-dist:{Wasserstein_D.data.item():06.4f} Seen-loss:{c_errG.data.item():06.4f} "
                          f"Unseen-loss:{unseenc_errG.data.item():06.4f} Loss-div:{loss_lz.item():06.4f} "
                          f"W-diff:{cfxs * fake_D.data.item():06.4f}")


            # # Total loss with Contrast Loss
            # # ---------------------------------------------
            # # todo dxm unseen_cls contrastive loss  dxm
            # unseen_ca = get_unseen_class_ids(self.opt.dataset, self.opt.classes_split)
            # fake_unseen_l_id = torch.tensor([unseen_ca[i] for i in fake_unseen_l])
            # contras_loss = self.contras_criterion(
            #     torch.cat([fake_unseen_f.cuda(), input_res, diff_res], 0),
            #     torch.cat([fake_unseen_l_id.cuda(), input_label, diff_label], 0))
            # contras_loss *= 0.0005
            #
            # if self.epoch >= 5:
            #     errG = G_cost + c_errG + loss_lz + unseenc_errG + contras_loss
